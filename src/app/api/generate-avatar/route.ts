...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ image_url: null, error: "Failed to generate image" }, { status: response.status });
    }
    ...
    return NextResponse.json({ image_url: null, error: "Internal server error" }, { status: 500 });
  }
}
